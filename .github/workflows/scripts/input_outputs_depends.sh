...
# === Catálogo via API (obrigatório) ===
CATALOG_URL="${CATALOG_URL:-}"                 # EXIGIDO
CATALOG_API_TOKEN="${CATALOG_API_TOKEN:-}"     # opcional (Bearer)
CATALOG_TIMEOUT="${CATALOG_TIMEOUT:-20}"       # segundos
CATALOG_INSECURE="${CATALOG_INSECURE:-0}"      # 1 = ignorar TLS
CATALOG_FILE="${CATALOG_FILE:-catalog_response.json}"  # só para salvar/depurar o retorno

# Debug de matching (1 = ligado)
DEBUG_MATCH="${DEBUG_MATCH:-0}"

# ========= Prechecks =========
need(){ command -v "$1" >/dev/null 2>&1 || { echo "Erro: '$1' não encontrado no PATH"; exit 1; }; }
need jq; need awk; need sed; need grep; need curl
...
[[ -n "$CATALOG_URL"   ]] || { echo "Erro: CATALOG_URL não definido (obrigatório)"; exit 1; }

mkdir -p "biblioteca/$COSTCENTER"

# ========= Buscar catálogo (API) =========
echo "Baixando catálogo de: $CATALOG_URL"
...
[[ "$CATALOG_INSECURE" == "1" ]] && curl_opts+=( -k )

curl "${curl_opts[@]}" "$CATALOG_URL" | tee "$CATALOG_FILE" >/dev/null
jq -e '.[].metadata.name' "$CATALOG_FILE" >/dev/null || { echo "Erro: resposta da API não contém .[].metadata.name"; exit 1; }

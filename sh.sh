...
for var in "${inputs[@]}"; do
    ...
    for resource in "${resource_names[@]}"; do
        ...
        resource_no_prefix=$(echo "$resource" | sed 's/terraform-//' | sed 's/azurerm-//' | xargs)

        IFS='-' read -ra resource_parts <<< "$resource_no_prefix"
        var_parts_count=$(echo "$var_core" | awk -F'-' '{print NF}')

        for ((i=0; i<=${#resource_parts[@]}-var_parts_count; i++)); do
            candidate_core="${resource_parts[@]:i:var_parts_count}"
            candidate_core=$(echo $candidate_core | tr ' ' '-')
            if [[ "$candidate_core" == "$var_core" ]]; then
